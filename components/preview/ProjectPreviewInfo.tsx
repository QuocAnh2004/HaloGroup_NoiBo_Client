...
import { formatProjectDates, getCurrentUser } from "../../utils";
          ...
          <div className="space-y-6">
            ...
            {techCategories.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {cat.items.map((tech) => (
                        <span
                          key={tech}
                          className="bg-white text-slate-600 px-3 py-1.5 rounded-lg text-xs font-medium shadow-sm border border-slate-100/50"
                        >
                          {tech}
                        </span>
                      ))}
              ...
              <p className="text-slate-300 italic font-light">
                Chưa cập nhật thông tin công nghệ.
              </p>
            )}
          </div>
    ...
    <p className="text-slate-300 italic font-light">
          ...
          <div className="flex gap-10 pt-4">
            {project.githubUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-3 text-slate-400 hover:text-slate-900 transition-colors"
              >
                <Github size={24} />
        ...
        <div className="lg:col-span-5 flex flex-col justify-center items-center lg:items-end">
          <div className="relative group">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center space-y-2">
              <span className="text-5xl md:text-7xl font-medium text-slate-900">
                {project.progress}%
              </span>
          ...
          <div className="grid grid-cols-2 gap-4 mt-16 w-full md:max-w-sm">
            ...
            <div
              className={`p-6 bg-slate-50 rounded-[32px] space-y-2 ${timeStatus.color === "text-rose-500" ? "bg-rose-50" : ""}`}
            >
              ...
              <div className={`text-2xl font-medium ${timeStatus.color}`}>
                {timeStatus.value}
              </div>
              <div
                className={`text-[10px] ${timeStatus.color === "text-rose-500" ? "text-rose-400" : "text-slate-400"}`}
              >
                {timeStatus.label}
              </div>
            </div>
      ...
      <div className="mt-32 pt-16 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div className="space-y-4">
          ...
          <div
            className={`text-lg font-medium ${project.status === ProjectStatus.COMPLETED ? "text-emerald-500" : "text-indigo-600"}`}
          >
            {project.status}
          </div>
        </div>
