...
//   ensureUserInList: (userId: string) => Promise<void>;
// };
...
//   // thêm user người mới chat vào list
//   // Thêm hàm này vào ChatContext
//           const ensureUserInList = async (userId: string) => {
//             // Nếu đã có trong list rồi thì thôi
//             if (chatUsers.find((u) => u.userId === userId)) return;
...
//                   user: users[0].name,
//                   name: users[0].name,
//                   avatar: users[0].avatar || "/chat_page/assets/images/users/user-default.png",
//                 };
...
//               }
//             } catch (e) {
//               console.error("❌ Không load được user:", e);
//             }
//           };
...
//   // // ✅ THÊM: Nếu người gửi chưa có trong chatUsers → tự động thêm vào list
//   // // Dùng functional update để đọc state mới nhất, tránh stale closure
//   // setChatUsers((prev) => {
//   //   const alreadyExists = prev.some((u) => u.userId === otherUserId);
//   //   if (alreadyExists) return prev;

//   //   // Fetch thông tin user rồi thêm vào (async bên ngoài setter)
//   //   authFetch("/messages/users/by-ids", {
//   //     method: "POST",
//   //     body: JSON.stringify({ ids: [otherUserId] }),
//   //   })
//   //     .then((users: any[]) => {
//   //       if (users.length > 0) {
//   //         const newUser: ChatUser = {
...
//   //         };
//   //         setChatUsers((p) => {
//   //           if (p.some((u) => u.userId === newUser.userId)) return p;
//   //           return [newUser, ...p]; // thêm vào đầu list
//   //         });
//   //       }
//   //     })
//   //     .catch((e) => console.error("❌ Không load được user mới:", e));

//   //   return prev; // trả về nguyên trước, setter async sẽ cập nhật sau
//   // });
...
//   // Nếu đang không chat với người này → bỏ qua append message (chỉ cần hiện trong list)
//   if (
//     !activeUserIdRef.current ||
//     String(activeUserIdRef.current) !== otherUserId
//   ) {
...
socket.on("receive_message", (msg: any) => {
  ...
  setMessages((prev) => {
    ...
    socket.on("notify", (data: any) => {
      ...
      // Server gửi: { type: "NEW_MESSAGE", from: sender_id, message: {...} }
      if (data?.type !== "NEW_MESSAGE") return;

      const senderId = String(data?.from ?? "");
      if (!senderId || senderId === String(myId)) return;

      // ✅ Bước 1: Join room với người gửi để nhận receive_message tiếp theo
      const pairId = [String(myId), senderId].sort().join("-");
      socket.emit("join_chat", pairId);
      console.log("🔗 Auto-joined room from notify:", pairId);

      // ✅ Bước 2: Append tin nhắn vào UI ngay (dùng data.message từ notify)
      const msg = data?.message;
      if (msg) {
        const sid = String(msg?.sender_id ?? senderId);
        const rid = String(msg?.receiver_id ?? myId);

        // Nếu đang active với người này thì append message
        if (String(activeUserIdRef.current) === senderId) {
          setMessages((prev) => {
      ...
      // ✅ Bước 3: Thêm sender vào chatUsers nếu chưa có
      setChatUsers((prev) => {
        if (prev.some((u) => u.userId === senderId)) return prev;

        authFetch("/messages/users/by-ids", {
          method: "POST",
          body: JSON.stringify({ ids: [senderId] }),
        })
          .then((users: any[]) => {
            if (users.length > 0) {
              const newUser: ChatUser = {
          ...
          .catch((e) => console.error("❌ Không load được user từ notify:", e));
