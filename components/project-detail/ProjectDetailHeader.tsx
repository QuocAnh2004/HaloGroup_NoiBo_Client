...
const ProjectDetailHeader: React.FC<ProjectDetailHeaderProps> = ({
  ...
  const { project, updateProject, totalProgress, isSyncing } =
    useProjectDetail();
  const navigate = useNavigate();
  const [completionError, setCompletionError] = useState<string | null>(null);

  const isLate =
    project.status === ProjectStatus.IN_PROGRESS && isOverdue(project.dueDate);
          ...
          <div className="flex items-center justify-between w-full md:w-auto gap-3">
            <div className="flex items-center gap-3 md:gap-5 min-w-0 flex-1">
              ...
              <div className="flex flex-col gap-y-1">
                ...
                <div
                  className={`flex items-center gap-1.5 text-[10px] md:text-xs font-light ${isLate ? "text-rose-400" : "text-slate-400"}`}
                >
                  <Calendar
                    size={15}
                    className={isLate ? "text-rose-300" : "text-slate-300"}
                  />
          ...
          <div className="flex items-center justify-between w-full md:w-auto md:justify-end gap-4 border-t border-slate-50 md:border-0 md:pt-0">
            ...
            <div className="relative flex items-center gap-3">
              ...
              >
                ...
                <span
                  className={`text-[10px] uppercase font-medium tracking-wider ${isCompleted ? "text-emerald-500" : "text-slate-400"}`}
                >
            ...
            <div className="flex items-center gap-2 justify-end">
              {/* Priority */}
              <span
                className={`p-2.5 rounded-lg text-[10px] font-medium uppercase tracking-wider flex items-center gap-1.5 shadow-sm ${priorityColors[project.priority]}`}
              >
            ...
            {/* Desktop Preview Button */}
              <div className="px-8 pt-6 flex intems-centers gap-3 p-6">
                    <button
                onClick={() => setOpenInfoModal(true)}
                className="flex items-center px-4 py-2 bg-indigo-600 text-white rounded-xl md:rounded-2xl shadow-sm hover:shadow-lg hover:bg-indigo-700 transition-all duration-300 active:scale-95 text-[11px] md:text-xs font-medium shrink-0"
              >
                <Plus size={14} className="mr-2 md:w-4 md:h-4" />
