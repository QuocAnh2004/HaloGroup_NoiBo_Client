...
import { getCurrentUser, isOverdue } from "../../utils";
...
const ProjectDetailInfo: React.FC = () => {
  const { project, updateProject, deleteProject, isReadOnly } =
    useProjectDetail();
  ...
  useEffect(() => {
    ...
    project.techStack.forEach((tech) => {
      ...
      else if (tech.startsWith("be:"))
        techByCategory.be.push(tech.substring(3));
      else if (tech.startsWith("db:"))
        techByCategory.db.push(tech.substring(3));
      else if (tech.startsWith("do:"))
        techByCategory.do.push(tech.substring(3));
      else {
    ...
    <div className="space-y-10 relative pb-20">
      ...
      <div className="bg-white rounded-[40px] p-8 shadow-sm space-y-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            <label
              className={`text-[10px] font-medium uppercase tracking-widest flex items-center gap-2 ml-1 ${isLate ? "text-rose-500" : "text-slate-400"}`}
            >
